...
// FIA stewards impose time penalties in the individual decision documents (and
// only some reach the classification's penalty footer), so read them from the
// same documents we scan for fines. Handles "a 10 second time penalty", the
// "2 x 5 second" repeat form, and the reversed "time penalty of 5 seconds".
// The imposed penalty is stated in the document's Decision line; the Reason that
// follows often repeats a "N second" figure (a gap, a limit) that must not be
// counted, so scope parsing to the Decision. Falls back to the whole text when
// there is no Decision heading (short fixtures).
function decisionClause(text) {
  const flat = String(text || '').replace(/\s+/g, ' ');
  return flat.match(/\bDecision\b[:\s](.*?)(?:\bReason\b|$)/i)?.[1] || flat;
}

export function timePenaltyFromText(text) {
  const decision = decisionClause(text);
  let total = 0;
  for (const match of decision.matchAll(/(?:(\d+)\s*x\s*)?(\d+)\s*second(?:s)?\s*time\s+penalty/gi)) {
    total += (match[1] ? Number(match[1]) : 1) * Number(match[2]);
  }
  for (const match of decision.matchAll(/time\s+penalty\s+of\s+(\d+)\s*second/gi)) {
    total += Number(match[1]);
  }
  // Drive-through / stop-go penalties served or converted after the race are
  // stated as an added time, e.g. "Drive through penalty ... (20 seconds added)".
  // Only read these when no explicit "N second time penalty" was found, so a doc
  // that states both the penalty and its "(N seconds added)" is not doubled.
  if (total === 0) {
    for (const match of decision.matchAll(/\(?\s*(\d+)\s*seconds?\s*added\b/gi)) {
      total += Number(match[1]);
    }
  }
  return total;
...
// Grid penalties are stated in the pre-race decision documents, not derivable
// from grid-vs-qualifying (a back-of-grid start barely moves a car that already
// qualified last, yet still carries the full penalty). Read the two capped forms
// — back-of-grid and pit-lane starts count as the 10-place maximum — plus an
// explicit "N grid place penalty". scoreGridPenalty applies Martin's cap.
export function gridPenaltyFromText(text) {
  const decision = decisionClause(text);
  if (/no penalty applied/i.test(decision)) return 0;
  // A grid-position drop "for the next Race" is served at a later event, so it
  // does not affect this race's grid; only a pit-lane or back-of-grid start does.
  if (/(?:start(?:ing)?|relegat\w+)\s+(?:the\s+race\s+)?(?:to\s+|from\s+)?the\s+back\s+of\s+the\s+(?:starting\s+)?grid/i.test(decision)) return 10;
  if (/start\s+(?:the\s+race\s+)?from\s+the\s+pit\s*lane/i.test(decision)) return 10;
  if (/next\s+Race/i.test(decision)) return 0;
  const match = decision.match(/(\d+)\s*grid\s*(?:place|position)s?\s*penalty/i)
    || decision.match(/grid\s*(?:place|position)\s*penalty\s*of\s*(\d+)/i)
    || decision.match(/drop(?:ped)?\s*(?:of|by)?\s*(\d+)\s*grid\s*(?:place|position)/i);
...
function fiaCachePath(url) {
  const name = url.split('/').pop().replace(/[^\w.-]/g, '_');
  return join(FIA_TEXT_CACHE_DIR, `${name}.txt`);
}

export async function fetchPdfText(url, options = {}) {
  const cachePath = fiaCachePath(url);
  ...
  if (text) {
    mkdirSync(FIA_TEXT_CACHE_DIR, { recursive: true });
