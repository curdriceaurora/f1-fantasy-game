...
export function deriveFinishingPositions(raceResultRows, fieldSize = raceResultRows.length) {
  ...
  for (const row of disqualified) {
    positions.set(row.driver_number, fieldSize);
  }
  return positions;
}
...
export function normalizeRaceWeekend(calendarRace, fetchedRace, fineSummary = { drivers: {}, teams: {}, documents: [] }) {
  ...
  // Use the canonical entry-list size for the shared-last rule. OpenF1 result
  // rows can be incomplete, so their count must not define "last place".
  const fieldSize = driverDirectory.size;
  const finishingPositions = deriveFinishingPositions(fetchedRace.raceResultRows, fieldSize);
  ...
  // Grid penalties are published only in the FIA starting-grid footer. OpenF1's
  // race-control feed carries none at all, so gridPenaltyMap is a fallback that
  // has never produced a value; the document wins whenever it is present.
  const fiaGridPenalties = fia.gridPenaltyPlaces || {};
  const hasFiaGridPenalties = Boolean(fia.gridPenaltyPlaces);
  const fiaPitLaneGridPenalties = fia.pitLaneGridPenalties || {};
  const fiaPenalties = fia.penaltySeconds || {};
  const fiaSprint = fia.sprintPositions || {};
  const sprintPositions = deriveFinishingPositions(fetchedRace.sprintResultRows, fieldSize);
  ...
  for (const directoryEntry of driverDirectory.values()) {
    ...
    const pitLaneGridPenalty = fiaPitLaneGridPenalties[directoryEntry.canonicalDriverId] || null;
    const gridPenaltyPlaces = hasFiaGridPenalties
      ? (fiaGridPenalties[directoryEntry.canonicalDriverId] || 0)
      : (gridPenaltyMap.get(directoryEntry.driver_number) || 0);
    if (pitLaneGridPenalty?.status === 'resolved'
      && pitLaneGridPenalty.places !== gridPenaltyPlaces) {
      throw new Error(`Resolved pit-lane grid penalty for ${directoryEntry.canonicalDriverId} is ${pitLaneGridPenalty.places}, but the scored value is ${gridPenaltyPlaces}`);
    }
    if (pitLaneGridPenalty?.status === 'unresolved' && gridPenaltyPlaces !== 0) {
      throw new Error(`Unresolved pit-lane grid penalty for ${directoryEntry.canonicalDriverId} cannot carry ${gridPenaltyPlaces} scored places`);
    }
    drivers[directoryEntry.canonicalDriverId] = {
      ...
      // Martin scores a qualifying DSQ in the DNQ column, not the band of the
      // back-row slot the driver actually starts from, so the band needs its own
      // flag rather than being inferred from gridStart.
      qualifyingDsq: qualifyingDisqualified.has(directoryEntry.canonicalDriverId),
      racePosition,
      ...
      gridPenaltyPlaces,
      ...(pitLaneGridPenalty ? { pitLaneGridPenalty } : {}),
  ...
  // The fastest lap is derived from OpenF1 lap times, independent of the result.
  // A disqualified car cannot hold it — the FIA would never award one — so if our
  // data lands the fastest lap on a DSQ'd driver, that is a source anomaly to
  // surface for review rather than silently score (or silently drop).
  const fastestLapHolder = Object.values(drivers).find((driver) => driver.fastestLap);
  if (fastestLapHolder?.dsq) {
    throw new Error(`Fastest lap for ${calendarRace.id} resolved to disqualified driver "${fastestLapHolder.driverId}"; a DSQ'd car cannot hold the fastest lap — review the source data before scoring.`);
  }
