...
// Exact, not merely sufficient. An unexpected field means a scoring input was
// added on one side without being added to the comparison, so it would be
// computed and published without ever being checked against Martin.
function unexpectedFields(fields, expected) {
  return Object.keys(fields || {}).filter((field) => !expected.includes(field));
}
  ...
  for (const [raceId, race] of Object.entries(races || {})) {
    ...
    for (const [kind, record, expected] of [
      ...
      for (const [id, fields] of Object.entries(record || {})) {
        ...
        const unexpected = unexpectedFields(fields, expected);
        if (unexpected.length) {
          problems.push(
            `${where}: ${kind} ${id} has unexpected field(s): ${unexpected.join(', ')}`
            + ' — add them to the expected schema so they are compared',
          );
        }
      }
    }
