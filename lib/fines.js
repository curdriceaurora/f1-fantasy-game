...
// Both the reader below and the remover in activeFineFromText must see exactly
// the same spans, so they share one pattern. Every gap is \s+: steward PDFs put
// line breaks and non-breaking spaces between these words, and a literal space
// silently matches nothing.
const PARTIAL_SUSPENSION_SOURCE = '€\\s*([\\d][\\d.,]*)\\s+of\\s+which\\s+€\\s*([\\d][\\d.,]*)\\s+(?:is|are)\\s+suspend';

function partialSuspensionPattern() {
  return new RegExp(PARTIAL_SUSPENSION_SOURCE, 'gi');
}

function partiallySuspendedFines(text) {
  return [...String(text || '').matchAll(partialSuspensionPattern())].map((match) => ({
...
export function activeFineFromText(text) {
  ...
  const partialPayable = partialSuspensions.reduce(
    (sum, { awarded, suspended }) => sum + Math.max(0, awarded - suspended),
    0,
  );
  // Remove the "€X of which €Y is suspended" spans already accounted for above,
  // so the general scan below sees only the remaining, independent fines rather
  // than re-reading the paired amounts (which it would net to zero) or dropping
  // a separate full fine published in the same document.
  const remainder = String(text || '').replace(partialSuspensionPattern(), ' ');
