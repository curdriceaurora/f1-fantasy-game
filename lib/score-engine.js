...
export function buildDriverContribution(driverId, raceDriver, raceContext) {
  ...
  components.push({
    ...
    // Position change scores against the improvement baseline, which equals the
    // grid start for everyone except drivers disqualified in qualifying — they are
    // measured from the last grid slot (field size). The factual grid start is kept
    // for the qualifying band. Falls back to gridStart when no baseline is set.
    points: scorePositionChange(raceDriver.improvementGrid ?? raceDriver.gridStart, raceDriver.racePosition),
  });
