...
// classifies at the back — so only disqualified cars are dropped.
export function deriveFinishingPositions(raceResultRows) {
  const classified = raceResultRows.filter((row) => row.position != null);
...
export async function fetchRaceWeekend(calendarRace) {
  ...
  if (calendarRace.isSprintWeekend && !sprintSession) {
    throw new Error(`OpenF1 sprint session missing for sprint weekend ${calendarRace.id}`);
  }
  ...
  if (calendarRace.isSprintWeekend && !sprintResultRows.length) {
    throw new Error(`OpenF1 sprint results are missing for sprint weekend ${calendarRace.id}`);
  }
...
export function normalizeRaceWeekend(calendarRace, fetchedRace, fineSummary = { drivers: {}, teams: {}, documents: [] }) {
  ...
  if (calendarRace.isSprintWeekend && (!Array.isArray(fetchedRace.sprintResultRows) || !fetchedRace.sprintResultRows.length)) {
    throw new Error(`OpenF1 sprint results are missing for sprint weekend ${calendarRace.id}`);
  }
  ...
  const timePenaltyMap = parseTimePenaltyMessages(fetchedRace.raceTimePenaltyMessages || []);
  ...
  for (const directoryEntry of driverDirectory.values()) {
    ...
    drivers[directoryEntry.canonicalDriverId] = {
      ...
      gridStart: fiaGrid[directoryEntry.canonicalDriverId] ?? gridStarts.get(directoryEntry.driver_number) ?? null,
      racePosition: fiaFinish[directoryEntry.canonicalDriverId] ?? finishingPositions.get(directoryEntry.driver_number) ?? null,
      ...
      gridPenaltyPlaces: directoryEntry.canonicalDriverId in fiaFinish
