...
export async function fetchRaceResults(race, options = {}) {
  ...
  for (const [driverId, resolution] of Object.entries(pitLaneGridPenalties || {})) {
    const numberedGridPenaltyPlaces = gridPenalties.get(driverId);
    if (numberedGridPenaltyPlaces != null) {
      pitLaneGridPenalties[driverId] = { ...resolution, numberedGridPenaltyPlaces };
    }
    // The pit-lane decision is the representation that determines the start. A
    // simultaneous numbered penalty is retained on the marker for reconciliation
    // to flag, rather than aborting ingestion of an otherwise valid FIA grid.
    if (resolution.status === 'resolved') gridPenalties.set(driverId, resolution.places);
    else gridPenalties.delete(driverId);
  }
...
export function parseStartingGrid(text) {
  ...
  if (pitLaneStart >= 0) {
    ...
    const unknownCarNumbers = new Set();
    ...
    for (let index = 1; index < pitLaneLines.length; index += 1) {
      ...
      const driver = carNumber && resolveDriverByCarNumber(carNumber);
      if (carNumber) seenCarNumbers.add(String(Number(carNumber)));
      if (driver && !seenDriverIds.has(driver.id)) {
        pitLaneDrivers.push({ driver, carNumber: Number(carNumber) });
        seenDriverIds.add(driver.id);
      } else if (carNumber && !driver) {
        unknownCarNumbers.add(String(Number(carNumber)));
      }
    }

    if (!seenCarNumbers.size) {
      throw new Error('FIA starting grid contains a pit-lane section but no starter entries could be parsed');
    }
    if (unknownCarNumbers.size) {
      throw new Error(`Unable to map pit-lane starter car number(s): ${[...unknownCarNumbers].join(', ')}`);
    }
...
export async function resolvePitLaneGridPenalties(race, pitLaneStarters, options = {}) {
  ...
  let urls;
  try {
    urls = await fetchFiaDecisionUrlsImpl(race, options);
  } catch (error) {
    return Object.fromEntries([...pitLaneStarters.keys()].map((driverId) => [driverId, {
      status: 'unresolved',
  ...
  for (const [driverId, carNumber] of pitLaneStarters) {
    ...
    resolutions[driverId] = resolved[0]
      || matches[0]
      || (unreadableUrls.length
        ? { status: 'unresolved', reason: 'decision-candidates-unreadable', candidateUrls: unreadableUrls }
