...
function parseTimePenaltyMessages(messages) {
  ...
  for (const event of parseTimePenaltyEvents(messages)) {
    // A served notice repeats the original amount and used to be counted as a
    // second penalty in the OpenF1-only fallback. The FIA footer remains the
    // authoritative unserved ledger whenever it is available.
    if (event.served) continue;
    penalties.set(
      event.driverNumber,
      (penalties.get(event.driverNumber) || 0) + event.seconds,
    );
  }
  return penalties;
}
