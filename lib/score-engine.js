...
const QUALIFYING_MATRIX = {
  ...
  'No Hoper': { pole: 15, top5: 12, top10: 9, top14: 6, top18: 3, rest: 0, dnq: -2 },
};

// The six capability ranks Martin assigns, exported so a check against his
// TDriver table can assert it saw all of them rather than silently covering a
// subset (see scripts/verify-qualifying-matrix.mjs).
export const QUALIFYING_RANKS = Object.freeze(Object.keys(QUALIFYING_MATRIX));
...
function occupantDetails(driverId, result) {
  const canonical = driverById(driverId);
  return {
    driverId,
    name: canonical?.fullName || result?.fullName || result?.name || driverId,
  };
}

export function buildDriverContribution(driverId, raceDriver, raceContext, sessionContext = {}) {
  ...
  const qualifyingDriver = sessionContext.qualifyingDriver || raceDriver;
  const sprintDriver = Object.hasOwn(sessionContext, 'sprintDriver')
    ? sessionContext.sprintDriver
    : raceDriver;
  const qualifyingOccupantId = sessionContext.occupants?.qualifying;
  const raceOccupantId = sessionContext.occupants?.race;
  // When the occupant changes after qualifying, the replacement's race grid is
  // not the qualifying occupant's result. Score that session from qualifying;
  // unchanged seats retain the existing official-grid behavior.
  const startPosition = qualifyingOccupantId && qualifyingOccupantId !== raceOccupantId
    ? qualifyingDriver.qualifyingPosition
    : qualifyingDriver.gridStart ?? qualifyingDriver.qualifyingPosition;
  // A driver disqualified from qualifying is scored in the DNQ column, not the band
  // of the back-row slot they actually start from. Keep that rule attached to the
  // qualifying-session occupant when a seat changes hands between sessions.
  const qualifyingPositionForBand = qualifyingDriver.qualifyingDsq ? null : startPosition;
  const components = [
    {
      label: qualifyingDriver.qualifyingDsq
        ? `Qualifying (DSQ, ruled DNQ, grid P${startPosition ?? '—'})`
        : `Qualifying (grid P${startPosition ?? 'DNQ'})`,
      points: scoreQualifying(driver.rank, qualifyingPositionForBand),
    },
  ];

  if (sprintDriver?.sprintPosition) {
    components.push({
      label: `Sprint P${sprintDriver.sprintPosition}`,
      points: scoreSprintFinish(sprintDriver.sprintPosition),
    });
  }
  ...
  return {
    ...
    rank: driver.rank,
    seatId: sessionContext.seat?.id || null,
    sessionOccupants: sessionContext.occupants || {
      qualifying: driverId,
      sprint: driverId,
      race: driverId,
    },
    sessionOccupantDetails: {
      qualifying: occupantDetails(
        sessionContext.occupants?.qualifying || driverId,
        qualifyingDriver,
      ),
      sprint: occupantDetails(
        sessionContext.occupants && Object.hasOwn(sessionContext.occupants, 'sprint')
          ? sessionContext.occupants.sprint
          : driverId,
        sprintDriver,
      ),
      race: occupantDetails(
        sessionContext.occupants?.race || driverId,
        raceDriver,
      ),
    },
    qualifyingPosition: qualifyingDriver.qualifyingPosition,
    qualifyingGridStart: startPosition,
    sprintPosition: sprintDriver?.sprintPosition ?? null,
...
function requireSessionDriver(normalizedRace, seat, session, occupantDriverId) {
  const result = normalizedRace.drivers[occupantDriverId];
  if (!result) {
    throw new Error(
      `No normalized ${session} result found for occupant "${occupantDriverId}" in seat ${seat.id}`,
    );
  }
  return result;
}

export function buildSeatContribution(driverId, normalizedRace, raceContext, inferredRaceOccupants = {}) {
  const resolved = resolveSeatOccupants(
    driverId,
    normalizedRace.seatOccupants,
    inferredRaceOccupants,
  );
  const { seat } = resolved;
  const occupants = {
    ...resolved.occupants,
    sprint: normalizedRace.sprintWeekend ? resolved.occupants.sprint : null,
  };
  const qualifyingDriver = requireSessionDriver(normalizedRace, seat, 'qualifying', occupants.qualifying);
  const sprintDriver = normalizedRace.sprintWeekend
    ? requireSessionDriver(normalizedRace, seat, 'sprint', occupants.sprint)
    : null;
  const raceDriver = requireSessionDriver(normalizedRace, seat, 'race', occupants.race);

  return buildDriverContribution(driverId, raceDriver, raceContext, {
    seat,
    occupants,
    qualifyingDriver,
    sprintDriver,
...
export function scoreFantasyTeam(entry, normalizedRace) {
  ...
  const inferredRaceOccupants = inferRaceSeatOccupants(
    normalizedRace.teams,
    normalizedRace.seatOccupants,
    normalizedRace.raceId,
  );
  const driverContributions = entry.selectedDriverIds.map((driverId) =>
    buildSeatContribution(driverId, normalizedRace, raceContext, inferredRaceOccupants),
  );
  ...
  const constructorContributions = entry.selectedConstructorIds.map((teamId) => {
    ...
    if (!raceTeam) {
      const team = teamById(teamId);
      throw new Error(`No normalized constructor result found for ${team?.name || teamId}`);
    }
    // Score both stable team seats so a substitute inherits the seat's driver
    // category and its designated 3x/2x constructor weighting. Building directly
    // from raceTeam.driverIds would treat a reserve as an unknown fantasy driver.
    const constructorDrivers = seatsForTeam(teamId).map((seat) =>
      buildSeatContribution(
        seat.ownerDriverId,
        normalizedRace,
        raceContext,
        inferredRaceOccupants,
