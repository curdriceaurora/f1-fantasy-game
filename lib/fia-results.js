...
export async function fetchRaceResults(race, options = {}) {
  ...
  const { timePenalties, gridPenalties } = await fetchDecisionPenalties(race, options);

  return {
    finishingPositions: Object.fromEntries(classification.positions),
    // Prefer the per-document time penalties: the classification footer only
    // lists a subset (a served penalty may never reach it), and Martin charges
    // every time penalty as points.
    penaltySeconds: Object.keys(timePenalties).length
      ? timePenalties
      : Object.fromEntries(classification.penalties),
    gridPenaltyPlaces: gridPenalties,
