...
export function normalizeRaceWeekend(calendarRace, fetchedRace, fineSummary = { drivers: {}, teams: {}, documents: [] }, options = {}) {
  ...
  const canonicalizeDriverNameImpl = options.canonicalizeDriverName || canonicalizeDriverName;
  const canonicalizeTeamNameImpl = options.canonicalizeTeamName || canonicalizeTeamName;
  const resolveTeamImpl = options.resolveTeam || resolveTeam;

  const driverDirectory = new Map();
  for (const driver of fetchedRace.drivers) {
    const canonicalDriver = canonicalizeDriverNameImpl(driver);
    if (!canonicalDriver) {
      throw new Error(`Unable to map OpenF1 driver "${driver.full_name}" to canonical constants`);
    }
    const canonicalTeam = canonicalizeTeamNameImpl(driver.team_name) || resolveTeamImpl(canonicalDriver.team);
    if (!canonicalTeam) {
  ...
  for (const directoryEntry of driverDirectory.values()) {
    ...
    drivers[directoryEntry.canonicalDriverId] = {
      ...
      gridPenaltyPlaces: hasFiaGridPenalties
