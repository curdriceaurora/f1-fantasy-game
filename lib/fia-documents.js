...
const PENALTY_URL_SIGNALS = [
  '_infringement_', '_decision_', '_appeal_', '_right_of_review_',
];
...
export function isPotentialFineDocument(url) {
  ...
  return PENALTY_URL_SIGNALS.some((s) => lower.includes(s));
}
...
export async function discoverPotentialPenaltyPdfs(race, options = {}) {
  ...
  return allUrls.filter(isPotentialFineDocument);
}
