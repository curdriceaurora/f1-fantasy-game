...
export function assertCompleteSources(sources) {
  ...
  for (const [raceId, source] of Object.entries(sources)) {
    const drivers = Object.keys(source.race.drivers).length;
    const teams = Object.keys(source.race.teams).length;
    if (drivers === EXPECTED_DRIVERS && teams === EXPECTED_TEAMS) continue;
    problems.push(
      `${raceId} (${source.sheet} of ${source.workbook}): parsed ${drivers}/${EXPECTED_DRIVERS} drivers`
      + ` and ${teams}/${EXPECTED_TEAMS} constructors`,
    );
  }
  return problems;
}
  ...
  for (const [raceId, before] of Object.entries(previousRaces || {})) {
    ...
    for (const kind of ['drivers', 'teams']) {
      for (const id of Object.keys(before[kind] || {})) {
        if (!after[kind]?.[id]) losses.push(`${raceId}: ${kind.slice(0, -1)} ${id} dropped out of the ledger`);
      }
    }
  }
  return losses;
