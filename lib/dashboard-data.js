...
export function buildWeekOverWeekDelta(entries, standingsRows) {
...
function enrichConstructor(teamId) {
...
// Presentation flags for the public schedule. Keyed by calendar id so a race
// keeps its national flag even when hosted elsewhere (e.g. Bahrain at Sepang).
const RACE_FLAG_BY_ID = {
  australia: '🇦🇺', china: '🇨🇳', japan: '🇯🇵', 'saudi-arabia': '🇸🇦',
  miami: '🇺🇸', canada: '🇨🇦', monaco: '🇲🇨', 'barcelona-catalunya': '🇪🇸',
  austria: '🇦🇹', 'great-britain': '🇬🇧', belgium: '🇧🇪', hungary: '🇭🇺',
  netherlands: '🇳🇱', italy: '🇮🇹', spain: '🇪🇸', azerbaijan: '🇦🇿',
  bahrain: '🇧🇭', singapore: '🇸🇬', 'united-states': '🇺🇸', mexico: '🇲🇽',
  brazil: '🇧🇷', 'las-vegas': '🇺🇸', qatar: '🇶🇦', 'abu-dhabi': '🇦🇪',
};

// The public race calendar, sourced entirely from the calendar config so the
// schedule grid stays in sync with scoring (dates, rounds, sprint weekends, and
// cancelled/rescheduled status) instead of being hand-maintained in the page.
export function loadCalendarScheduleData() {
  const races = loadCalendar().map((race) => ({
    id: race.id,
    round: race.round ?? null,
    name: race.name,
    date: race.date,
    flag: RACE_FLAG_BY_ID[race.id] || '🏁',
    isSprintWeekend: Boolean(race.isSprintWeekend),
    status: race.status || 'scheduled',
    venue: race.venue || null,
  }));
  return { races, activeCount: races.filter((race) => race.status !== 'cancelled').length };
}
...
export function loadTeamDetail(teamId) {
  ...
  return {
    ...
    seasonSelections: {
      ...
      driverChampion: enrichDriver(entry.predictions?.driverChampion) || null,
      constructorChampion: enrichConstructor(entry.predictions?.constructorChampion) || null,
      colapintoBestFinish: entry.predictions?.colapintoBestFinish ?? null,
      colapintoDriver: enrichDriver('franco-colapinto'),
    },
    standings: {
      totalPoints: standing?.totalPoints ?? stored?.totalPoints ?? 0,
      completedRaces: standing?.completedRaces ?? stored?.completedRaces ?? 0,
      latestRacePoints: standing?.latestRacePoints ?? stored?.latestRacePoints ?? 0,
