...
export function timePenaltyFromText(text) {
  ...
  let total = 0;
  for (const match of flat.matchAll(/(?:(\d+)\s*x\s*)?(\d+)\s*second(?:s)?\s*time\s+penalty/gi)) {
    total += (match[1] ? Number(match[1]) : 1) * Number(match[2]);
  }
  for (const match of flat.matchAll(/time\s+penalty\s+of\s+(\d+)\s*second/gi)) {
    total += Number(match[1]);
  }
  // Drive-through and stop/go penalties served or converted after the race are
  // stated as an added time, e.g. "Drive through penalty ... (20 seconds added)".
  // Only read these when no explicit "N second time penalty" was found, so a
  // doc that states both the penalty and its "(N seconds added)" is not doubled.
  if (total === 0) {
    for (const match of flat.matchAll(/\(?\s*(\d+)\s*seconds?\s*added\b/gi)) {
      total += Number(match[1]);
    }
  }
  return total;
...
export function gridPenaltyFromText(text) {
  const flat = String(text || '').replace(/\s+/g, ' ');
  if (/(?:start(?:ing)?|relegat\w+)\s+(?:the\s+race\s+)?(?:to\s+|from\s+)?the\s+back\s+of\s+the\s+grid/i.test(flat)) return 10;
  if (/start\s+(?:the\s+race\s+)?from\s+the\s+pit\s*lane/i.test(flat)) return 10;
  const match = flat.match(/(\d+)\s*grid\s*(?:place|position)s?\s*penalty/i)
    || flat.match(/grid\s*(?:place|position)\s*penalty\s*of\s*(\d+)/i)
    || flat.match(/drop(?:ped)?\s*(?:of|by)?\s*(\d+)\s*grid\s*(?:place|position)/i);
