...
export function buildDriverContribution(driverId, raceDriver, raceContext) {
  ...
  // of the back-row slot they actually start from: Martin's Australia sheet gives
  // Verstappen (Contender) -13, the DNQ value, rather than the -9 of 19th-22nd. The
  // factual gridStart is still reported, and the improvement baseline is separate.
  const qualifyingPositionForBand = raceDriver.qualifyingDsq ? null : startPosition;
  const components = [
    {
      label: raceDriver.qualifyingDsq
