...
import { loadCalendar, loadEntries, normalizedRacePath, readJson, scoredRacePath, seasonPaths, standingsPath, teamScorePath, writeJson, removeFile } from './season-store.js';
...
export function rebuildScoreboard() {
  ...
  const normalizedRaces = calendar
    ...
    // Order races chronologically. Date is the stable key: the calendar `round`
    // can be renumbered (e.g. a race rescheduled to a later date), which would
    // otherwise conflict with the round baked into already-scored snapshots.
  ...
  const validTeamIds = new Set(entries.map((entry) => entry.teamId));
  const scoredTeamsDir = seasonPaths().scoredTeams;
  if (existsSync(scoredTeamsDir)) {
    for (const fileName of readdirSync(scoredTeamsDir)) {
      if (!fileName.endsWith('.json')) continue;
      const teamId = fileName.replace(/\.json$/, '');
      if (!validTeamIds.has(teamId)) {
        removeFile(teamScorePath(teamId));
      }
    }
  }
