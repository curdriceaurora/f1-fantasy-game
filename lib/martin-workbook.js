...
export function assertCompleteSources(sources) {
  ...
  for (const [raceId, source] of Object.entries(sources)) {
    const where = `${raceId} (${source.sheet} of ${source.workbook})`;
    ...
    if (drivers !== EXPECTED_DRIVERS || teams !== EXPECTED_TEAMS) {
      problems.push(`${where}: parsed ${drivers}/${EXPECTED_DRIVERS} drivers and ${teams}/${EXPECTED_TEAMS} constructors`);
    }
    for (const [kind, record, expected] of [
      ['driver', source.race.drivers, EXPECTED_DRIVER_FIELDS],
      ['team', source.race.teams, EXPECTED_TEAM_FIELDS],
    ]) {
